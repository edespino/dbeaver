...
public class CloudberryExternalTable extends PostgreTable {
    ...
    private CloudberryExternalTableUriLocationsHandler uriLocationsHandler;
    ...
    public CloudberryExternalTable(PostgreSchema catalog) {
        super(catalog);
        this.uriLocationsHandler = new CloudberryExternalTableUriLocationsHandler("", '\n');
        this.encoding = CloudberryCharacterSet.UNICODE_8BIT.getCharacterSetValue();
    ...
    public void setUriLocations(String lineFeedSeparatedUriLocations) {
        this.uriLocationsHandler =
                new CloudberryExternalTableUriLocationsHandler(lineFeedSeparatedUriLocations, '\n');
    }
    ...
    @Property(viewable = true, editable = true, updatable = true, order = 25,
            listProvider = CloudberryCharacterSetProvider.class)
    ...
    public static class ExternalTableFormatTypeProvider implements IPropertyValueListProvider<CloudberryExternalTable> {
        public boolean allowCustomValue() {
            return false;
        }

        public Object[] getPossibleValues(CloudberryExternalTable object) {
            Predicate<FormatType> excludeCustomTypes = formatType -> !formatType.equals(FormatType.b);

            return Arrays.stream(CloudberryExternalTable.FormatType.values())
                    .filter(excludeCustomTypes)
                    .map(CloudberryExternalTable.FormatType::getValue).toArray();
        }
    }

    public static class CloudberryCharacterSetProvider implements IPropertyValueListProvider<CloudberryExternalTable> {
        public boolean allowCustomValue() {
            return false;
        }

        public Object[] getPossibleValues(CloudberryExternalTable object) {
            return Arrays.stream(CloudberryCharacterSet.values())
                    .map(CloudberryCharacterSet::getCharacterSetValue).toArray();
        }
    }

    public static class ExternalTableUriLocationsRenderer
            implements IPropertyValueTransformer<CloudberryExternalTable, String> {
        @Override
        public String transform(CloudberryExternalTable cloudberryExternalTable,
                                String commaSeparatedListUriLocations) throws IllegalArgumentException {
            return cloudberryExternalTable.uriLocationsHandler.getLineFeedSeparatedList();
        }
    }
}
