...
public class CloudberrySchema extends PostgreSchema {

    public CloudberrySchema(PostgreDatabase owner, String name, JDBCResultSet resultSet) throws SQLException {
    ...
    protected ProceduresCache createProceduresCache() {
        return new CloudberryFunctionsCache();
    }

    @NotNull
    @Override
    protected TableCache createTableCache() {
        return new CloudberryTableCache();
    }

    @NotNull
    @Override
    public CloudberryDataSource getDataSource() {
        return (CloudberryDataSource) super.getDataSource();
    }

    @Association
    public Collection<? extends JDBCTable> getExternalTables(DBRProgressMonitor monitor) throws DBException {
        return new ArrayList<>(getTableCache().getTypedObjects(monitor, this, CloudberryExternalTable.class));
    }
    ...
    public List<? extends PostgreTable> getTables(DBRProgressMonitor monitor) throws DBException {
        ...
        return postgreTables.stream()
            .filter(e -> !(e instanceof CloudberryExternalTable))
    ...
    public List<? extends PostgreTable> getForeignTables(DBRProgressMonitor monitor) throws DBException {
        ...
        CloudberryDataSource dataSource = getDataSource();
        if (dataSource.isServerVersionAtLeast(7, 0) && dataSource.isHasAccessToExttable()) {
            // Starting Cloudberry version 7 external tables are marked as foreign tables.
    ...
    public class CloudberryTableCache extends TableCache {

        private boolean before7version;

        CloudberryTableCache() {
            super();
        }
        ...
        public JDBCStatement prepareLookupStatement(@NotNull JDBCSession session,
            ...
            CloudberryDataSource dataSource = getDataSource();
            boolean cloudberryVersionAtLeast5 = dataSource.isCloudberryVersionAtLeast(5, 0);
            String uriLocationColumn = cloudberryVersionAtLeast5 ? "urilocation" : "location";
            String execLocationColumn = cloudberryVersionAtLeast5 ? "execlocation" : "location";
            ...
            before7version = !dataSource.isCloudberryVersionAtLeast(7, 0);
    ...
    public class CloudberryFunctionsCache extends ProceduresCache {
        CloudberryFunctionsCache() {
            super();
        }
        ...
        @Override
        protected CloudberryFunction fetchObject(@NotNull JDBCSession session,
                                                @NotNull PostgreSchema owner,
                                                @NotNull JDBCResultSet dbResult) {
            return new CloudberryFunction(session.getProgressMonitor(), owner, dbResult);
        }
    }
}
