 ...
 * CloudberryTable
 */
public class CloudberryTable extends PostgreTableRegular {

    private static final Log log = Log.getLog(CloudberryTable.class);
    ...
    public CloudberryTable(PostgreSchema catalog) {
        super(catalog);
    }

    public CloudberryTable(PostgreSchema catalog, ResultSet dbResult) {
        ...
        if (dataSource instanceof CloudberryDataSource) {
            if (((CloudberryDataSource) dataSource).isCloudberryVersionAtLeast(7, 0)) {
    ...
    public void appendTableModifiers(DBRProgressMonitor monitor, StringBuilder ddl) {
        try {
            ...
            PostgreDataSource dataSource = getDataSource();
            if (dataSource instanceof CloudberryDataSource) {
                // Read partitions DDLs separately starting Cloudberry 7
                readPartitionInfo = !((CloudberryDataSource) dataSource).isCloudberryVersionAtLeast(7, 0);
            }
            CloudberryUtils.addObjectModifiersToDDL(
                monitor,
                ddl,
                this,
                distributionColumns,
