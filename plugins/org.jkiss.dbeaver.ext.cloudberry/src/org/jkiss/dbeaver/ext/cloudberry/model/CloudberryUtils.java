...
public class CloudberryUtils {

    @Nullable
    static int[] readDistributedColumns(@NotNull DBRProgressMonitor monitor, @NotNull PostgreTableReal table) throws DBCException {
        try (JDBCSession session = DBUtils.openMetaSession(monitor, table, "Read Cloudberry table distributed columns")) {
            try (JDBCStatement dbStat = session.createStatement()) {
                if (((CloudberryDataSource) table.getDataSource()).isCloudberryVersionAtLeast(6, 0)) {
    ...
    private static String getPartitionData(@NotNull DBRProgressMonitor monitor, @NotNull PostgreTableReal table) throws DBCException {
        try (JDBCSession session = DBUtils.openMetaSession(monitor, table, "Read Cloudberry table partition data")) {
            try (JDBCStatement dbStat = session.createStatement()) {
    ...
    static void addObjectModifiersToDDL(
        ...
        ddl.append("\nDISTRIBUTED ");
        if (supportsReplicatedDistribution && table.isPersisted() && CloudberryUtils.isDistributedByReplicated(monitor, table)) {
        ...
        if (addPartitionInfo) {
            String partitionData = table.isPersisted() ? CloudberryUtils.getPartitionData(monitor, table) : null;
