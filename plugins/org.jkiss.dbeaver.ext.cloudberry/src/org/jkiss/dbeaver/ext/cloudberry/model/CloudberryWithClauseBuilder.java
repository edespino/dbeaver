...
public class CloudberryWithClauseBuilder {
    ...
    public static String generateWithClause(PostgreTable table, PostgreTableBase tableBase) {
        ...
        if (tableSupportsAndHasOids(table) && tableIsCloudberryWithRelOptions(table, tableBase)) {
            ...
            withClauseBuilder.append("\nWITH (\n\tOIDS=").append(table.isHasOids() ? "TRUE" : "FALSE");
            withClauseBuilder.append("\n)");
        } else if (tableIsCloudberryWithRelOptions(table, tableBase)) {
    ...
    private static boolean tableIsCloudberryWithRelOptions(PostgreTable table, PostgreTableBase tableBase) {
        return table instanceof CloudberryTable && tableBase.getRelOptions() != null;
    }
}
