...
public class CloudberryFunctionTest {
    @Mock
...
//        Mockito.when(mockSchema.getConstraintCache()).thenReturn(mockConstraintCache);
        ...
        Mockito.when(mockDataSource.getServerType()).thenReturn(new PostgreServerCloudberry(mockDataSource));
...
//        Mockito.when(mockDatabase.getDefaultContext(Mockito.any(), Mockito.anyBoolean())).thenReturn(mockContext);
    ...
    @Test
    public void onCreationWithDbResult_whenCloudberryVersionIsSixAndAbove_thenExecutionLocationIsLoaded()
            throws SQLException {
        Mockito.when(mockResults.getString("proexeclocation")).thenReturn("a");

        withCloudberryVersion6AndAbove(() -> {
            CloudberryFunction function = new CloudberryFunction(mockMonitor, mockSchema, mockResults);
            Assert.assertEquals(CloudberryFunction.FunctionExecLocation.a, function.getExecutionLocation());
        });
    }

    @Test
    public void onCreationWithDbResult_whenCloudberryVersionIsBelowSix_thenExecutionLocationIsNull()
            throws SQLException {
        withCloudberryVersionLessThan6(() -> {
            CloudberryFunction function = new CloudberryFunction(mockMonitor, mockSchema, mockResults);
    ...
    @Test
    public void onCreation_whenCloudberryVersionIsSixAndAbove_thenExecutionLocationDefaultsToANY() {
        withCloudberryVersion6AndAbove(() -> {
            CloudberryFunction function = new CloudberryFunction(mockSchema);
            Assert.assertEquals(CloudberryFunction.FunctionExecLocation.a, function.getExecutionLocation());
        });
    }

    @Test
    public void onCreation_whenCloudberryVersionIsBelowSix_thenExecutionLocationIsNotSet() {
        withCloudberryVersionLessThan6(() -> {
            CloudberryFunction function = new CloudberryFunction(mockSchema);
            Assert.assertNull(function.getExecutionLocation());
        });
    ...
    private void withCloudberryVersion6AndAbove(Runnable testCase) {
        setCloudberryToVersion6();
        testCase.run();
    }

    private void withCloudberryVersionLessThan6(Runnable testCase) {
        setCloudberryToVersionLessThan6();
        testCase.run();
    }

    private void setCloudberryToVersionLessThan6() {
        // Cloudberry 6 runs on Postgre 9.4.x
        Mockito.when(mockDataSource.isServerVersionAtLeast(9, 4)).thenReturn(false);
    }

    private void setCloudberryToVersion6() {
        // Cloudberry 6 runs on Postgre 9.4.x
